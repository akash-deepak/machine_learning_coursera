...
%-------------------------------------------------------------------------
%Feedforward and Backpropogation for every input 
%and Calculating unregularized cost
%-------------------------------------------------------------------------
for i = 1:m,
  a1 = [1, X(i,:)];
  z2 = a1 * Theta1';
  ...
  a3 = a3';
  delta3 = a3 - p(:,i);
  product1 = Theta2' * delta3;
  product1 = product1(2:end);
  delta2 = (product1).*sigmoidGradient(z2');
  bigdelta2 = bigdelta2 + delta3*a2;
  bigdelta1 = bigdelta1 + delta2*a1;
  for j = 1:num_labels,
      cost1 += p(j,i) * log(a3(j)) + (1 - p(j,i))*log(1 - a3(j));
  endfor
endfor
Theta1_grad(:,1) = bigdelta1(:,1)/m;
for i = 2:input_layer_size+1,
  Theta1_grad(:,i) = bigdelta1(:,i)/m + (Theta1(:,i)*lambda)/m;
endfor
Theta2_grad(:,1) = bigdelta2(:,1)/m; 
for i = 2:hidden_layer_size+1,
  Theta2_grad(:,i) = bigdelta2(:,i)/m + (Theta2(:,i)*lambda)/m;
endfor
%--------------------------------------------------------------------------
...
cost1 = -(cost1)/m;
%--------------------------------------------------------------------------
%Computing regularized part of the cost
%--------------------------------------------------------------------------
for j = 1:hidden_layer_size,
  for k = 1:input_layer_size,
    cost2 += Theta1(j,k+1)^2;
  endfor
endfor
for j = 1:num_labels,
  for k = 1:hidden_layer_size,
    cost2 += Theta2(j,k+1)^2 ;
  endfor
endfor
